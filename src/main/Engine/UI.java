...
public class UI{
    ...
    public void draw(Graphics2D g2) {
        ...
        g2.fillRect(0,0, this.dp.getWidth(), DrawingPanel.ECART);

        g2.setFont(new Font("Arial", Font.BOLD, 20));
        g2.setColor(Color.BLACK);
        double divider = (this.dp.getJeu().getTour().getHeros().getPDV() / 10) * 100; // 10 max / 10      //10 max / 5
        this.healthBarWidth = (int) (divider * this.maxHealthWidth / 100);
        if (dp.getJeu().getTour().getCurrentLevel().isKeyFound() && imageChanged == false){
            changeImage();
        }
        g2.drawString("Niveau : " + dp.getJeu().getTour().getLevelNumber(), 10, DrawingPanel.TILE_SIZE/2+5);
        g2.drawImage(keyImage, dp.getScreenWidth()-DrawingPanel.TILE_SIZE, 0, DrawingPanel.TILE_SIZE,DrawingPanel.TILE_SIZE , null);

        g2.drawImage(healthDecorationImage, dp.getScreenWidth()/2 - (60), 5, 150,42 , null);
        g2.drawImage(healthImage, dp.getScreenWidth()/2+20 - (maxHealthWidth/2-12), 5, healthBarWidth,42 , null);
    }
